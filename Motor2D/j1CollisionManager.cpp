...
bool j1CollisionManager::Awake(pugi::xml_node & config)
{
	...
	matrix[COLLIDER_PLAYER][COLLIDER_WOOD_DOOR] = true;

	matrix[COLLIDER_ZELDA_ARROW][COLLIDER_WALL] = true;
	...
	//Enemy collisions
	matrix[COLLIDER_ENEMY][COLLIDER_WOOD_DOOR] = true;
	...
	matrix[COLLIDER_BOMB_EXPLOSION][COLLIDER_BUSH] = true;

	matrix[COLLIDER_BOMB][COLLIDER_BOMB_EXPLOSION] = true;
	matrix[COLLIDER_BOMB][COLLIDER_PLAYER] = true;
	matrix[COLLIDER_BOMB][COLLIDER_ENEMY] = true;

	matrix[COLLIDER_INTERRUPTOR][COLLIDER_LINK_ARROW] = true;
