...
bool j1Fonts::CleanUp()
{
	...
	for (std::list<TTF_Font*>::iterator item = fonts.begin(); item != fonts.cend(); item++)
		TTF_CloseFont(*item);
...
SDL_Texture* j1Fonts::Print(const char* text, int new_line, SDL_Color color, TTF_Font* font)
{
	SDL_Texture* ret = NULL;
	SDL_Surface* surface = TTF_RenderText_Blended_Wrapped((font) ? font : default, text, color, new_line);
